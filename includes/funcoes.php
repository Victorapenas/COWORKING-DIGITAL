...
function getIcone(string $nome): string
{
    ...
    $icons = [
        ...
        // --- Ícones de Substituição de Emojis ---
        'aceno' => "<svg xmlns='http://www.w3.org/2000/svg' $style viewBox='0 0 24 24'><path d='M18 11V6a2 2 0 0 0-4 0v5a2 2 0 0 1-4 0V4a2 2 0 0 0-4 0v13c0 3 4 5 6 6s6-3 6-5v-7z'></path></svg>", // Mão acenando (simplificado)
        'refresh' => "<svg xmlns='http://www.w3.org/2000/svg' $style viewBox='0 0 24 24'><polyline points='23 4 23 10 17 10'></polyline><polyline points='1 20 1 14 7 14'></polyline><path d='M3.51 9a9 9 0 0 1 14.85-3.36L23 10M1 14l4.64 4.36A9 9 0 0 0 20.49 15'></path></svg>",
        'close' => "<svg xmlns='http://www.w3.org/2000/svg' $style viewBox='0 0 24 24'><line x1='18' y1='6' x2='6' y2='18'></line><line x1='6' y1='6' x2='18' y2='18'></line></svg>",
        'play' => "<svg xmlns='http://www.w3.org/2000/svg' $style viewBox='0 0 24 24'><polygon points='5 3 19 12 5 21 5 3'></polygon></svg>",
        'pause' => "<svg xmlns='http://www.w3.org/2000/svg' $style viewBox='0 0 24 24'><rect x='6' y='4' width='4' height='16'></rect><rect x='14' y='4' width='4' height='16'></rect></svg>",
        'check_circle' => "<svg xmlns='http://www.w3.org/2000/svg' $style viewBox='0 0 24 24'><path d='M22 11.08V12a10 10 0 1 1-5.93-9.14'></path><polyline points='22 4 12 14.01 9 11.01'></polyline></svg>",
        'upload' => "<svg xmlns='http://www.w3.org/2000/svg' $style viewBox='0 0 24 24'><path d='M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4'></path><polyline points='17 8 12 3 7 8'></polyline><line x1='12' y1='3' x2='12' y2='15'></line></svg>",
        'confetti' => "<svg xmlns='http://www.w3.org/2000/svg' $style viewBox='0 0 24 24'><path d='M2 12h2'></path><path d='M20 12h2'></path><path d='M12 2v2'></path><path d='M12 20v2'></path><path d='M4.93 4.93l1.41 1.41'></path><path d='M17.66 17.66l1.41 1.41'></path><path d='M4.93 19.07l1.41-1.41'></path><path d='M17.66 6.34l1.41-1.41'></path></svg>" // Representação abstrata de festa
    ];
    return $icons[$nome] ?? '';
}
...
function processarDadosMembro(&$m)
{
    ...
    if (!empty($m['ultima_atividade'])) {
        ...
        if ($diff < 60)
            $m['atividade_txt'] = "Agora";
        elseif ($diff < 3600)
            $m['atividade_txt'] = floor($diff / 60) . " min atrás";
        elseif ($diff < 86400)
            $m['atividade_txt'] = floor($diff / 3600) . " h atrás";
        else
            $m['atividade_txt'] = floor($diff / 86400) . " dias atrás";
...
function getMembrosFiltrados(int $empresaId, string $filtro): array
{
    ...
    if ($filtro === 'GESTOR')
        $sql .= " AND p.nome = 'GESTOR' AND u.ativo = 1";
    elseif ($filtro === 'COLABORADOR')
        $sql .= " AND (p.nome = 'FUNCIONARIO' OR p.nome = 'COLABORADOR') AND u.ativo = 1";
    elseif ($filtro === 'ARQUIVADO')
        $sql .= " AND u.ativo = 0";
...
{
    $pdo = conectar_db();
    $m = $pdo->prepare("SELECT COUNT(*) FROM usuario WHERE empresa_id=? AND ativo=1");
    $m->execute([$empresaId]);
    $tm = $m->fetchColumn();
    $o = $pdo->prepare("SELECT COUNT(*) FROM usuario WHERE empresa_id=? AND ativo=1 AND ultima_atividade > (NOW() - INTERVAL 5 MINUTE)");
    $o->execute([$empresaId]);
    $to = $o->fetchColumn();
    $t = $pdo->prepare("SELECT SUM(CASE WHEN status='CONCLUIDA' THEN 1 ELSE 0 END) as c, SUM(CASE WHEN status IN ('PENDENTE','EM_ANDAMENTO') THEN 1 ELSE 0 END) as a FROM tarefa t JOIN usuario u ON t.responsavel_id=u.id WHERE u.empresa_id=?");
    $t->execute([$empresaId]);
    $st = $t->fetch(PDO::FETCH_ASSOC);
...
{
    $pdo = conectar_db();
    $st = $pdo->prepare("SELECT id, nome, email, cargo_detalhe, empresa_id FROM usuario WHERE id=?");
    $st->execute([$id]);
    return $st->fetch(PDO::FETCH_ASSOC) ?: null;
}
function atualizarUsuarioBasico($id, $n, $c)
{
    $pdo = conectar_db();
    $st = $pdo->prepare("UPDATE usuario SET nome=?, cargo_detalhe=? WHERE id=?");
    return $st->execute([$n, $c, $id]);
}
function buscarEmpresaPorId($id)
{
    $pdo = conectar_db();
    $st = $pdo->prepare("SELECT id, nome, logo_url, padrao_senha FROM empresa WHERE id=?");
    $st->execute([$id]);
    return $st->fetch(PDO::FETCH_ASSOC) ?: null;
}
function atualizarNomeEmpresa($id, $n)
{
    $pdo = conectar_db();
    $st = $pdo->prepare("UPDATE empresa SET nome=? WHERE id=?");
    return $st->execute([$n, $id]);
}
...
function getProjetoDetalhe(int $id): ?array
{
    ...
    $feitas = 0;
    foreach ($proj['tarefas_lista'] as $t)
